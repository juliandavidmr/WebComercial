...
(function ($) {
	...
	LanguageFilter = function LanguageFilter(element, options) {
	...
	LanguageFilter.prototype = {
		listen: function listen() {
			this.$element.on('keypress', $.proxy(this.keyup, this)).on('keyup', $.proxy(this.keyup, this));
		...
		keyup: function keyup(e) {
			...
			switch (e.keyCode) {
				...
				default:
					...
					if (e.which < 32 && e.which !== 8 // Backspace
					) {
   ...
   * @param query string
   */
		resultHandler: function resultHandler(query) {
		...
		autofill: function autofill(langCode, languageName) {
		...
		render: function render(langCode) {
   ...
   */
		filter: function filter(langCode, searchTerm) {
			// FIXME script is ISO 15924 code. We might need actual name of script.
			var matcher = new RegExp('^' + this.escapeRegex(searchTerm), 'i'),
			    languageName = this.options.languages[langCode];

			return matcher.test(languageName) || matcher.test($.uls.data.getAutonym(langCode)) || matcher.test(langCode) || matcher.test($.uls.data.getScript(langCode));
		},
