...
(function ($) {
	...
	LanguageCategoryDisplay = function LanguageCategoryDisplay(element, options) {
	...
	LanguageCategoryDisplay.prototype = {
		...
		append: function append(langCode, regionCode) {
   ...
   */
		addToRegion: function addToRegion(langCode, region) {
			var i,
			    regionCode,
			    $li,
			    $column,
			...
			for (i = 0; i < regions.length; i++) {
				...
				$li = $('<li>').data('code', langCode).attr({
					lang: langCode,
					dir: $.uls.data.getDir(langCode)
				}).append($('<a>').prop('title', language).html(langName));
   ...
   */
		getColumn: function getColumn(regionCode, forceNew) {
		...
		render: function render() {
			...
			$.each($.uls.data.regiongroups, function (regionCode) {
				...
				$sectionTitle = $('<h3>').attr('data-i18n', 'uls-region-' + regionCode).addClass('eleven columns uls-lcd-region-section uls-lcd-region-title offset-by-one').text(regions[regionCode]);
		...
		quicklist: function quicklist() {
			var quickList, $quickListSection, $quickListSectionTitle, i, $column, langCode, language, langName, $li;
			...
			$quickListSectionTitle = $('<h3>').attr('data-i18n', 'uls-common-languages').addClass('eleven columns uls-lcd-region-section uls-lcd-region-title offset-by-one').text('Common languages'); // This is placeholder text if jquery.i18n not present
		...
		noResults: function noResults() {
			...
			$suggestions.find('h3').data('i18n', 'uls-no-results-suggestion-title').text('You may be interested in:').i18n();
