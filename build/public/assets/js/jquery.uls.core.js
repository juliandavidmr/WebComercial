...
(function ($) {
  ...
  */
	ULS = function ULS(element, options) {
   ...
   */
		listen: function listen() {
			...
			uls.$menu.on('keypress', $.proxy(this.keypress, this)).on('keyup', $.proxy(this.keyup, this));
			...
			this.$menu.find('.uls-region, .uls-region-link').regionselector({
				$target: lcd,
				languages: uls.languages,
				success: function success(regionfilter) {
   ...
   * @param langCode
   */
		select: function select(langCode) {
			this.hide();
